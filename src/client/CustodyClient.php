...
class CustodyClient implements CustodyApi
{
    private  $config;

    public function __construct(Config $config){
        $config->setCustodyPublicKey( RsaUtils::FormatterPublicKey($config->getCustodyPublicKey()) );
    ...
    private function doRequest($path, $method, $params = array()){
        ...
        if(!$body){
            $result->setCode(-2);
            $result->setMsg("json decode custody response false, raw body:{$body}");
            return $result;
        }
        ...
        $decrypt = RsaUtils::Decrypt($data["data"], $this->config->getCustodyPublicKey());
        if(!$decrypt){
            $result->setCode(-4);
            $result->setMsg("rsa decode custody response false, raw body:{$data['data']}");
            return $result;
        }
     ...
     * @param int $lastCustodyId custody Platform withdrawal id, return 100 larger than last custody id
     * @return withdraw list
     */
    public function SyncWithdrawList($lastCustodyId = 0){
        $params = array(
            'max_id'=>$lastCustodyId,
        );
